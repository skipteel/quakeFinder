...
var g = svg.append('g').selectAll('.label').data(gradientData).enter();
...
g.append('text')
  ...
  .attr('transform', function (d, i) {
    if (i === 0) {
      return (
        'translate(' + (parseFloat(d.xPosition) * w + 14) + ',' + (h - 22) + ')'
      );
    } else if (i === 1) {
      return (
        'translate(' + (parseFloat(d.xPosition) * w + 45) + ',' + (h - 22) + ')'
      );
    } else if (i === 2) {
      return (
        'translate(' + (parseFloat(d.xPosition) * w + 2) + ',' + (h - 22) + ')'
      );
    }
  })
  .style('fill', 'white')
...
g.append('text')
  ...
  .attr('transform', function (d, i) {
    if (i === 0) {
      return (
        'translate(' + (parseFloat(d.xPosition) * w + 13) + ',' + (h - 2) + ')'
      );
    } else if (i === 1) {
      return (
        'translate(' + (parseFloat(d.xPosition) * w + 40) + ',' + (h - 2) + ')'
      );
    } else if (i === 2) {
      return (
        'translate(' + (parseFloat(d.xPosition) * w + 20) + ',' + (h - 2) + ')'
      );
    }
  })
  .style('fill', 'white')
...
function resize() {
  ...
  window.proj.translate([width / 2, height / 2]).scale(width / 6);
    ...
    lapse = setInterval(function () {
      ...
      if (rotate) {
        slider.slider(
          'value',
          slider.slider('value') +
            ($('#timelapseSlider').slider('option', 'max') -
              $('#timelapseSlider').slider('option', 'min')) /
              75
        );
      } else {
        slider.slider(
...
function drawQuakes() {
  svg.selectAll('.point').style('visibility', function (d) {
    return d.milliseconds < $('#timelapseSlider').slider('value')
      ? 'visible'
      : 'hidden';
  });
  // Appends updated date/time below slider
  $('#timelapseDate').html(
    new Date($('#timelapseSlider').slider('value')).toUTCString().slice(0, 29)
  );
}
...
function renderGlobe(startTime, endTime, minmag, maxmag, svg) {
  ...
  function initializeSlider(quakes) {
    ...
    $('#daterange>h4').html(
      'Number of earthquakes,<br/>between ' +
        minmag +
        ' and ' +
        maxmag +
        ' magnitude,<br/>from ' +
        ...
        ':<br/>' +
        quakesNum +
        '<br/><br/><span id="credit">Earthquake catalog courtesy of the U.S. Geological Survey</span>'
    );
  }
  ...
  queue()
    ...
    .defer(function (f) {
      d3.json(
        'https://cors-anywhere.herokuapp.com/http://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&starttime=' +
          startTime +
          '&endtime=' +
          endTime +
          '&minmagnitude=' +
      ...
      .on('mouseover', function (place) {
        ...
        pulse = setInterval(function () {
          d3.select(that)
            ...
            .attr(
              'd',
              path.pointRadius(function (place) {
...
window.onload = function () {
    ...
    $('#from').datepicker({
      ...
      onClose: function (selectedDate) {
        ...
        $('#to').datepicker(
          'option',
          'maxDate',
          new Date(Math.min.apply(null, [tomorrow, maxDate]))
        );
      },
    });
  ...
  window.spacePath = d3.geo.path().projection(space).pointRadius(1);
  ...
  queue()
    ...
    .defer(function (f) {
      d3.xml(
        'https://cors-anywhere.herokuapp.com/http://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_day.atom'
      ).get(function (error, data) {
        $('#loading-div').css('display', 'none');
        f(error, data);
      });
  ...
  function render(error, world, tectonics, xml) {
    ...
    var ul = d3
      .select('#presentation')
      .append('ul')
      .attr('class', 'quakeslist');

    var header = ul
      .append('h4')
      .html(
        xml.getElementsByTagName('title')[0].textContent +
          "<br/><span id='updated'>Updated:" +
          new Date(
            xml.getElementsByTagName('updated')[0].textContent
          ).toString() +
          '<br/>Count: ' +
          entries.length +
          '</span>'
    ...
    var li = ul
      ...
      .style('font-size', '0.8em')
      .html(function (entry) {
        var cData = entry.getElementsByTagName('summary')[0].childNodes[0]
          .nodeValue;
        return (
          '<span id="quakeTitle">' +
          entry.getElementsByTagName('title')[0].textContent +
          '</span><br/><br/>Time: ' +
          cData.match('Time</dt><dd>(.*UTC)')[1] +
          '<br/>Location: ' +
          cData.match('Location</dt><dd>(.*)</dd><dt>Depth')[1] +
          '<br/>Depth: ' +
          cData.match('Depth</dt><dd>(.*)</dd></dl>')[1]
        );
      });
          ...
          var header =
            ul
              ...
              .html(
                xml.getElementsByTagName('title')[0].textContent +
                  "<br/><span id='updated'>Updated:" +
                  new Date(xml.getElementsByTagName('updated')[0].textContent)
              )
              .toString() + '</span>';

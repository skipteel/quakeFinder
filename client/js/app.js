...
function bindSubmitButton() {
  $('#submitButton').on('click', function() {
    ...
    var endTime = $('#to').val();
    var magVals = $('.slider').slider('option', 'values');
...
function rotateGlobe() {
  if (!rotate) {
    ...
    interval = setInterval(function() {
      var rot = window.proj.rotate();
      window.proj.rotate([(rot[0] += 0.4), (rot[1] += 0.01)]);
      refresh();
    }, 50);
  } else {
...
function renderGlobe(startTime, endTime, minmag, maxmag, svg) {
  ...
  function ready(error, quakes) {
    ...
    for (var i = 0; i < quakes.length; i++) {
      ...
      var area = quakes[i].properties.place;
      var milliseconds = quakes[i].properties.time;
      var date = new Date(milliseconds).toUTCString();
      var depth = quakes[i].geometry.coordinates[2];
      var url = quakes[i].properties.url;
      var id = quakes[i].id;
  ...
  $(function() {
    $('#from').datepicker({
      ...
      onClose: function(selectedDate) {
        ...
        var t = d.valueOf() + 2678400000; // Max of 31 days past the from date
